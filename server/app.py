...
class Newsletters(Resource):
    ...
    def post(self):
        ...
        response = make_response(
            newsletters_schema.dump(new_newsletter),
            201,
        )
...
class NewsletterByID(Resource):
    ...
    def patch(self, id):
        updated_newsletter = Newsletter.query.filter_by(id=id).first()
        for attr in request.form:
            setattr(updated_newsletter, attr, request.form[attr])

        db.session.add(updated_newsletter)
        db.session.commit()

        response = make_response(newsletter_schema.dump(updated_newsletter), 200)
